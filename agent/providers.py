...
def _write_provider_status(provider: str, cooldown_started_at: int, cooldown_eta: int,
    ...
    try:
        ...
        with open(PROVIDER_STATUS_PATH, "w") as f:
...
def anthropic_request(system_prompt: str, messages: list, max_tokens: int,
    ...
    """Call Anthropic API. Returns the full response dict (content blocks + stop_reason).
    ...
    try:
        ...
        if e.code >= 500:
            raise ProviderError(provider="anthropic", kind="network",
...
def local_model_request(system_prompt: str, messages: list, **kwargs) -> str:
    cfg = CONFIG.get("local_model", {})
    ...
    for provider in _provider_attempt_order(state):
        ...
        try:
            if provider == "anthropic":
                ...
                text = local_model_request(system_prompt, messages)
                result = {
                    "content": [{"type": "text", "text": text}],
                    "stop_reason": "end_turn",
                    "provider": "local",
