...
def test_safety_levels():
    """Correct tools are marked safe, confirm, or destructive."""
    safe_tools = ["read_calendar", "read_calendar_range", "read_reminders",
                  "web_search", "query_memory"]
    confirm_tools = ["add_memory"]
    ...
    for name in confirm_tools:
        assert get_safety_level(name) == CONFIRM, f"{name} should be confirm"
...
def test_unknown_tool_safety():
    """Unknown tools default to destructive (fail-closed safety gate)."""
    assert get_safety_level("nonexistent_tool") == DESTRUCTIVE
