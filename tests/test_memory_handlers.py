...
class MemoryHandlersTest(unittest.TestCase):
    def setUp(self):
        ...
        memory_handlers.MEMORY_DIR = memory_dir
        memory_handlers.MEMORY_FILE = memory_dir / "MEMORY.md"
        memory_handlers.LEGACY_MEMORY_FILE = base / "MEMORY.md"

    def tearDown(self):
        self.tmpdir.cleanup()

    def test_add_and_view_memory(self):
        # Patch out vector store import so we hit the MEMORY.md fallback path
        with patch.dict("sys.modules", {"agent.tools.memory_tools": None}):
