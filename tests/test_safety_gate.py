...
def test_safe_and_confirm_tools_all_pass():
    setup()
    # Both SAFE and CONFIRM tools pass the gate (execute immediately)
    passthrough_tools = ["read_calendar", "read_calendar_range", "read_reminders",
                         "web_search", "query_memory", "add_memory"]
    for tool in passthrough_tools:
        is_safe, prompt = check(tool, {})
        assert is_safe is True, f"{tool} should pass safety gate"
...
def test_confirm_punctuation_and_polite_tail():
    setup()
    for word in ["yes.", "yes please", "confirm, please"]:
        _save_pending({
            "tool_name": "query_memory",
            "tool_inputs": {"query": "test"},
            "prompt": "Test?",
        ...
        result = check_pending(word, chat_id=1, sender="+1 (555) 123-4567")
        assert result is not None, f"'{word}' should confirm"
        assert has_pending() is False


def test_pending_is_scoped_to_chat_and_sender():
    setup()
    _save_pending({
        "tool_name": "query_memory",
        "tool_inputs": {"query": "test"},
    ...
    assert check_pending("yes", chat_id=2, sender="+15551234567") is None
    assert has_pending() is True
    assert check_pending("yes", chat_id=1, sender="+15559876543") is None
    assert has_pending() is True
    assert check_pending("yes", chat_id=1, sender="+15551234567") is not None
    assert has_pending() is False
...
def test_non_confirmation_returns_none():
    ...
    assert has_pending() is False  # Pending action cleared when user moves on
